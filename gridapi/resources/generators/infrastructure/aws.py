...
class aws_infrastructure_generator(object):
    ...
    def generate_ami(self, region):
        ...
        builder['launch_block_device_mappings'] = [launch_block_device_mappings]
        provisioner = AutoDict()
        provisioner['execute_command'] = "echo 'packer' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
        ...
        provisioner['inline'] = [
            ...
            "cp -ax /etc/sysconfig/network-scripts/ifcfg-eth0 /etc/sysconfig/network-scripts/ifcfg-ens3",
        ...
        with open('result/{}/ami.json'.format(self.grid_name), 'w') as ami_json_file:
            json.dump(ami_config, ami_json_file)
        command = ['packer', 'build', '-machine-readable', 'result/{}/ami.json'.format(self.grid_name)]
    ...
    def generate_config(self):
        ...
        self.config['provider']['aws']['secret_key'] = self.aws_secret_access_key
        self.config['provider']['aws']['region'] = self.current_config.region
        self.config['variable']['region']['default'] = self.current_config.region
        self.config['variable']['grid_name']['default'] = self.current_config.parentgrid
        self.config['variable']['vpc_name']['default'] = '{}_{}_vpc'.format(self.current_config.parentgrid, self.current_config.region)
        self.config['variable']['key_name']['default'] = self.current_config.sshkey
        self.config['variable']['amis']['default'][self.current_config.region] = self.generate_ami(self.current_config.region)
        self.config['variable']['en_amis']['default'][self.current_config.region] = self.generate_ami_en(self.current_config.region, self.config['variable']['amis']['default'][self.current_config.region])
        with open('result/{}/infrastructure/config.tf'.format(self.grid_name), 'w') as config_file:
            json.dump(self.config, config_file)

    def generate_networking(self):
        self.networking['resource']['aws_vpc']['vpc']['cidr_block'] = '172.29.0.0/16'
        self.networking['resource']['aws_vpc']['vpc']['tags']['Name'] = '${var.vpc_name}'
        self.networking['resource']['aws_internet_gateway']['igw']['vpc_id'] = '${aws_vpc.vpc.id}'
        self.networking['resource']['aws_internet_gateway']['igw']['tags']['Name'] = '${var.vpc_name}'
        self.networking['resource']['aws_internet_gateway']['igw']['depends_on'] = ['aws_vpc.vpc']
        self.networking['resource']['aws_internet_gateway']['igw']['depends_on'] = ['aws_vpc.vpc']
        self.networking['resource']['aws_subnet']['grid_subnet']['vpc_id'] = '${aws_vpc.vpc.id}'
        self.networking['resource']['aws_subnet']['grid_subnet']['cidr_block'] = '172.29.0.0/20'
        self.networking['resource']['aws_subnet']['grid_subnet']['map_public_ip_on_launch'] = 'true'
        self.networking['resource']['aws_subnet']['grid_subnet']['tags']['Name'] = '${var.vpc_name}-${var.region}'
        self.networking['resource']['aws_route_table']['routes']['vpc_id'] = '${aws_vpc.vpc.id}'
        self.networking['resource']['aws_route_table']['routes']['route']['cidr_block'] = '0.0.0.0/0'
        self.networking['resource']['aws_route_table']['routes']['route']['gateway_id'] = '${aws_internet_gateway.igw.id}'
        self.networking['resource']['aws_route_table_association']['grid_route_table']['subnet_id'] = '${aws_subnet.grid_subnet.id}'
        self.networking['resource']['aws_route_table_association']['grid_route_table']['route_table_id'] = '${aws_route_table.routes.id}'
        for group in self.current_groups:
            if group.az is not None:
                self.networking['resource']['aws_subnet']['az_{}_subnet'.format(group.az)]['vpc_id'] = '${aws_vpc.vpc.id}'
                self.networking['resource']['aws_subnet']['az_{}_subnet'.format(group.az)]['cidr_block'] = az_nets[group.az]
                self.networking['resource']['aws_subnet']['az_{}_subnet'.format(group.az)]['map_public_ip_on_launch'] = 'true'
                self.networking['resource']['aws_subnet']['az_{}_subnet'.format(group.az)]['availability_zone'] = '{}{}'.format(self.current_config.region, group.az)
                self.networking['resource']['aws_subnet']['az_{}_subnet'.format(group.az)]['tags']['Name'] = '${{var.vpc_name}}-${{var.region}}{}'.format(group.az)
                self.networking['resource']['aws_route_table_association']['az_{}_route_table'.format(group.az)]['subnet_id'] = '${{aws_subnet.az_{}_subnet.id}}'.format(group.az)
                self.networking['resource']['aws_route_table_association']['az_{}_route_table'.format(group.az)]['route_table_id'] = '${aws_route_table.routes.id}'
    ...
    def generate_terminal(self):
        self.terminal['resource']['aws_instance']['terminal']['ami'] = '${lookup(var.amis,"${var.region}")}'
        self.terminal['resource']['aws_instance']['terminal']['instance_type'] = 'm3.large'
        self.terminal['resource']['aws_instance']['terminal']['key_name'] = '${var.key_name}'
        self.terminal['resource']['aws_instance']['terminal']['subnet_id'] = '${aws_subnet.grid_subnet.id}'
        self.terminal['resource']['aws_instance']['terminal']['security_groups'] = ['${aws_security_group.gridwide.id}', '${aws_security_group.terminal.id}', '${aws_security_group.publisher.id}']
        self.terminal['resource']['aws_instance']['terminal']['root_block_device']['volume_size'] = '50'
        self.terminal['resource']['aws_instance']['terminal']['tags']['Name'] = '${var.grid_name}_terminal'
        self.terminal['resource']['aws_instance']['terminal']['depends_on'] = ['aws_internet_gateway.igw']
        self.terminal['resource']['aws_instance']['terminal']['source_dest_check'] = 'false'
        self.terminal['resource']['aws_eip']['terminal']['instance'] = '${aws_instance.terminal.id}'
    ...
    def generate_masters(self):
        self.masters['resource']['aws_instance']['mesos_master']['count'] = '{}'.format(self.current_config.masters)
        ...
        self.masters['resource']['aws_instance']['mesos_master']['subnet_id'] = '${aws_subnet.grid_subnet.id}'
    ...
    def generate_groups(self):
        for group in self.current_groups:
            ...
            if group.spot_price is not None:
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['spot_price'] = '{}'.format(group.spot_price)
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['wait_for_fulfillment'] = 'true'
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['count'] = '{}'.format(group.slaves)
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['instance_type'] = '{}'.format(group.instance_type)
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['key_name'] = '${var.key_name}'
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['security_groups'] = ['${aws_security_group.gridwide.id}']
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['root_block_device']['volume_size'] = '{}'.format(group.disk_size)
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['tags']['Name'] = '${var.grid_name}_mesos_slave'
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['tags']['role'] = '{}_{}'.format(self.grid_name, group.role)
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['tags']['group'] = '{}'.format(group.name)
                if group.az is not None:
                    group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['subnet_id'] = '${{aws_subnet.az_{}_subnet.id}}'.format(group.az)
                else:
                    group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['subnet_id'] = '${aws_subnet.grid_subnet.id}'
                group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['depends_on'] = ['aws_internet_gateway.igw']
                if group.customhwconf is not None:
                    group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)].update(ast.literal_eval(group.customhwconf))
                if group.enhanced_networking:
                    group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['ami'] = '${lookup(var.en_amis,"${var.region}")}'
                else:
                    group_export['resource']['aws_spot_instance_request']['mesos_group_{}'.format(group.name)]['ami'] = '${lookup(var.amis,"${var.region}")}'
                with open('result/{}/infrastructure/{}.tf'.format(self.grid_name, group.name), 'w') as group_file:
                    json.dump(group_export, group_file)
            else:
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['count'] = '{}'.format(group.slaves)
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['instance_type'] = '{}'.format(group.instance_type)
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['key_name'] = '${var.key_name}'
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['security_groups'] = ['${aws_security_group.gridwide.id}']
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['root_block_device']['volume_size'] = '{}'.format(group.disk_size)
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['tags']['Name'] = '${var.grid_name}_mesos_slave'
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['tags']['role'] = '{}_{}'.format(self.grid_name, group.role)
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['tags']['group'] = '{}'.format(group.name)
                if group.az is not None:
                    group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['subnet_id'] = '${{aws_subnet.az_{}_subnet.id}}'.format(group.az)
                else:
                    group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['subnet_id'] = '${aws_subnet.grid_subnet.id}'
                group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['depends_on'] = ['aws_internet_gateway.igw']
                if group.customhwconf is not None:
                    group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)].update(ast.literal_eval(group.customhwconf))
                if group.enhanced_networking:
                    group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['ami'] = '${lookup(var.en_amis,"${var.region}")}'
                else:
                    group_export['resource']['aws_instance']['mesos_group_{}'.format(group.name)]['ami'] = '${lookup(var.amis,"${var.region}")}'
                with open('result/{}/infrastructure/{}.tf'.format(self.grid_name, group.name), 'w') as group_file:
                    json.dump(group_export, group_file)
