...
def calculate_mi_vars(func):
    ...
    def inner(*args, **kwargs):
        ...
        # attrs
        #if attrs.get('role', '') == 'control' or attrs.get('role', '') == 'mesos_master':
...
@calculate_mi_vars
def azure_instance(resource, module_name):
    raw_attrs = resource['primary']['attributes']
    name = raw_attrs['name']
    groups = []
    ...
    attrs = {
        ...
        'ansible_sudo_pass': raw_attrs['password'],
        'provider': 'azure',
    }

    if 'vip_address' in raw_attrs:
        attrs['public_ipv4'] = raw_attrs['vip_address']

    description_json = json.loads(raw_attrs.get('description', '{}'))

    # attrs specific to Mantl
    attrs.update({
        'ansible_ssh_host': attrs['public_ipv4'],
        'consul_dc': _clean_dc(description_json['dc']),
        'role': description_json['role']
    })
...
def gce_host(resource, module_name):
    ...
    attrs = {
        ...
        'alldisks': parse_attr_list(raw_attrs, 'disk'),
    ...
    try:
        attrs.update({
            'ansible_ssh_host': interfaces[0]['address'],
    ...
    # add groups based on attrs
    groups.extend('gce_image=' + disk['image'] for disk in attrs['alldisks'])
